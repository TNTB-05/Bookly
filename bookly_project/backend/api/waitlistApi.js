...
router.post('/', AuthMiddleware, async (req, res) => {
    try {
        ...
        const result = await database.addToWaitlist(userId, provider_id, service_id, date_from, date_to, time_from, time_to);
...
router.get('/', AuthMiddleware, async (req, res) => {
    try {
        const userId = req.user.userId;
        const entries = await database.getUserWaitlistEntries(userId);
...
router.delete('/:id', AuthMiddleware, async (req, res) => {
    try {
        const userId = req.user.userId;
        await database.cancelWaitlistEntry(parseInt(req.params.id), userId);
