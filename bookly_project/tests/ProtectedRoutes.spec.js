...
test.describe('Protected routes', () => {
  ...
  test('unauthenticated /dashboard redirects to customer login page', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/protected-routes/protected-01-dashboard-to-login.png' });
  });
  ...
  test('unauthenticated /dashboard/salon/:id redirects to customer login page', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/login/);
    await expect(page.getByRole('heading', { name: 'Bejelentkezés' })).toBeVisible();
    await page.screenshot({ path: 'screenshots/protected-routes/protected-02-salon-subpage-to-login.png' });
  });
  ...
  test('unauthenticated /ProvDash redirects to provider login page', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/protected-routes/protected-03-provdash-to-provider-login.png' });
  });
  ...
  test('unauthenticated /admin/dashboard redirects to admin login page', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/protected-routes/protected-04-admin-to-admin-login.png' });
  });
  ...
  test('unknown route redirects to home page', async ({ page }) => {
    ...
    await page.screenshot({ path: 'screenshots/protected-routes/protected-05-404-to-home.png' });
  });
  ...
  test('deeply nested unknown route redirects to home page', async ({ page }) => {
    ...
    await expect(page).toHaveURL('/');
    await page.waitForTimeout(5000);
    await page.screenshot({ path: 'screenshots/protected-routes/protected-06-deep-404-to-home.png' });
  });
  ...
  test('logged-in customer visiting /ProvDash is redirected to /dashboard', async ({ page }) => {
    ...
    await page.waitForTimeout(5000);
    await page.screenshot({ path: 'screenshots/protected-routes/protected-07-customer-to-provdash-blocked.png' });
  });
  ...
  test('logged-in customer visiting /admin/dashboard is redirected to /dashboard', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/dashboard/);
    await page.waitForTimeout(5000);
    await page.screenshot({ path: 'screenshots/protected-routes/protected-08-customer-to-admin-blocked.png' });
  });
